...
| `TensorRT/analysis.py` | **[레이어 변환 분석 → Markdown]** `TRT_layer_print.py` 의 `analyze()` 를 재사용해 원본 fusion·rename 내역을 `.md` 문서로 정리 → `model/TRT_layer_analysis.md` (요약표 + `model.N` 단계별 fusion 표 + rename 표 + TRT 신규/사라진 노드). 상세: [`TensorRT/analysis.md`](TensorRT/analysis.md) |
