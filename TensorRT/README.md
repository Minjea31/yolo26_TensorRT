...
| `compare_pt_trt.py` | **`.pt` 모듈(24개) ↔ TRT 커널(316개)** 집계 비교 — **표** → `pt_vs_trt.md` (repo 루트). 각 `model.N` 을 `내부 leaf 레이어 수 → TRT 커널 수` 로. `--onnx` 명시 시 가운데 ONNX 노드 수 추가(기본 꺼짐). 상세: [`compare_pt_trt.md`](compare_pt_trt.md) |
